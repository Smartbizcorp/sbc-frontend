...
type AdminInvestment = {
  ...
  // Si tu veux, tu peux aussi ajouter ici le tier
  // tier?: {
  //   id: number;
  //   name: string;
  //   durationDays: number;
...
export default function AdminInvestmentsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8">
      ...
      {/* HEADER */}
      <section className="bg-sbc-bgSoft/60 border border-sbc-border rounded-3xl p-6 md:p-7 shadow-[0_20px_55px_rgba(0,0,0,0.9)] backdrop-blur-lg flex flex-col gap-2">
        ...
        <p className="text-xs md:text-sm text-sbc-muted max-w-2xl leading-relaxed">
          Les demandes d&apos;investissement apparaissent d&apos;abord en statut{" "}
          <span className="text-sbc-gold font-semibold">PENDING</span>. Vous ne
          validez un palier qu&apos;après avoir confirmé la réception des fonds
          (Wave, etc.). Une fois validé, l&apos;investissement passe en statut{" "}
          <span className="text-sbc-gold font-semibold">ACTIVE</span>, puis peut
          être <span className="text-sbc-gold font-semibold">CLOSED</span> en
          fin de cycle.
        </p>
      </section>

      {/* FILTRES */}
      <section className="bg-sbc-bgSoft/50 border border-sbc-border rounded-3xl p-4 md:p-5 shadow-[0_16px_45px_rgba(0,0,0,0.85)] flex flex-col gap-3">
      ...
      <section className="bg-sbc-bgSoft/50 border border-sbc-border rounded-3xl p-5 md:p-6 shadow-[0_18px_55px_rgba(0,0,0,0.9)] flex flex-col gap-4">
                    ...
                    <Td>
                      {inv.status === "PENDING" ? (
                        <div className="flex flex-wrap gap-1">
                          ...
                          >
                            {actionLoadingId === inv.id ? "Rejet…" : "Rejeter"}
                          </button>
                        </div>
                      ) : inv.status === "ACTIVE" ? (
                        <button
        ...
        <p className="text-[10px] md:text-[11px] text-sbc-muted leading-relaxed">
          ...
          <span className="text-sbc-gold font-semibold">ACTIVE</span> qu&apos;une
          fois le dépôt effectivement reçu sur les comptes de Smart Business
          Corp. Les investissements{" "}
          <span className="text-sbc-gold font-semibold">REJECTED</span> peuvent
          faire l&apos;objet d&apos;un suivi manuel avec le client (explication,
          nouvelle tentative, etc.). Les investissements{" "}
...
function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="p-2 text-left font-medium border-b border-sbc-border/40 text-sbc-gold uppercase text-[10px] md:text-[11px]">
      {children}
    </th>
  );
}
...
function Td({
  ...
  return (
    <td
      className={`p-2 border-b border-sbc-border/30 text-[11px] md:text-xs ${className}`}
    >
      {children}
    </td>
  );
